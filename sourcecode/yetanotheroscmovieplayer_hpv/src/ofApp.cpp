...
void ofApp::setup(){
    receiver.setup(PORT);
    HPV::InitHPVEngine();
    HPV::hpv_log_set_level(HPV_LOG_LEVEL_WARNING);
    videoPlayer.init(HPV::NewPlayer());
                ...
                if(ofFile::doesFileExist(fileName)) {
                    ...
                    if(isImage(fileName)) {
                        ...
                        if(ofToLower(fileName.substr(fileName.find_last_of(".")+1))=="hpv") {
                            ...
                            cout<<"file "<<fileName<<" not compatible - use a .hpv\n";
                            type= NOTCOMPATIBLE;
                        }
                    }
                    state= FADEIN;
...
void ofApp::draw(){
    ...
    if((type==MOVIE)&&(state!=STOPPED)) {
        ...
        ofSetHexColor(0xFFFFFF);
        ofDrawBitmapString("file '"+fileName+"' not compatible - use a .hpv", 300, 300);
    }
    if(info) {
        ofSetHexColor(0xFFFFFF);
